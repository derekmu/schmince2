...
import com.schmince.game.model.sprites.MedicKitSprite;
...
public class GameModel implements GameModelInterface {
    ...
    private final List<SoundEvent> soundEvents = new ArrayList<SoundEvent>();
    ...
    public void onMedicKitUsed(Player player) {
        newSprite(new MedicKitSprite(player.getX(), player.getY()));
        soundEvents.add(new SoundEvent(SoundEventType.MedicKit));
    }

    public void onLocatorUsed(Player player) {
        soundEvents.add(new SoundEvent(SoundEventType.Locator));
    }
    ...
    public void getSprites(List<Sprite> spriteHolder) {
        synchronized (sprites) {
            for (int i = 0; i < sprites.size(); i++) {
                spriteHolder.add(sprites.get(i));
            }
            sprites.clear();
        }
    }
