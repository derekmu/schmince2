...
public class LabelDrawer implements GUIDrawer {
    ...
    public void draw(GUIItem item, GUIRenderer render) {
        ...
        if (label.Text != null && label.Text.length() > 0) {
            ...
            text.begin(label.Color.Red, label.Color.Green, label.Color.Blue, label.Color.Alpha, render.getVPOrthoMatrix());
            if (label.Align == Alignment.CenterLeft) {
                text.draw(
                        label.Text,
                        label.Bounds.x,
                        label.Bounds.centerY() - height / 2f,
                        0,
                        label.Bounds
                );
            } else if (label.Align == Alignment.BottomCenter) {
                text.draw(
                        label.Text,
                        label.Bounds.centerX() - width / 2f,
                        label.Bounds.bottom(),
                        0,
                        label.Bounds
                );
